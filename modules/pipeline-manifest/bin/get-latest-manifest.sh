...
OUTPUT=$(curl -s -H "Authorization: token $GITHUB_TOKEN" "https://api.github.com/repos/open-cluster-management/$PIPELINE_MANIFEST_REPO/git/trees/$PIPELINE_MANIFEST_LATEST_BRANCH?recursive=1")
...
$(curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github.v3.raw" https://raw.githubusercontent.com/open-cluster-management/$PIPELINE_MANIFEST_REPO/$PIPELINE_MANIFEST_LATEST_BRANCH/$LATEST --output $BASENAME)
